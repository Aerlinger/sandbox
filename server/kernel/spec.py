"""The Meta kernel spec for Jupyter"""
...
def make_metakernel_cmd(mod='metakernel', executable=None, extra_arguments=None, **kw):
  """Build Popen command list for launching a Meta kernel.

  Parameters
  ----------
  mod : str, optional (default 'metakernel')
      A string of an Meta module whose __main__ starts a Meta kernel
...
def install(kernel_spec_manager=None, user=False, kernel_name=KERNEL_NAME, display_name=None, prefix=None):
  """Install the Meta kernelspec for Jupyter
  ...
  kernel_name: str, optional
      ...
      This is needed for having multiple Meta kernels for different environments.
  prefix: str, optional
...
class InstallMetaKernelSpecApp(Application):
  """Dummy app wrapping argparse"""
  name = 'meta-kernel-install'
  ...
  def start(self):
    ...
    parser = argparse.ArgumentParser(prog=self.name,
                                     description="Install the meta kernel spec.")
    ...
    parser.add_argument('--name', type=str, default=KERNEL_NAME,
                        help="Specify a name for the kernelspec."
                             " This is needed to have multiple Meta kernels at the same time.")
    parser.add_argument('--display-name', type=str,
                        help="Specify the display name for the kernelspec."
                             " This is helpful when you have multiple Meta kernels.")
...
if __name__ == '__main__':
  InstallMetaKernelSpecApp.launch_instance()
