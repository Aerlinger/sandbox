...
# local imports
from server.io.iostream import IOPubThread
from server.connection.heartbeat import Heartbeat
from server.kernel.metakernel import MetaKernel
from server.kernel.parent_poller import ParentPollerUnix, ParentPollerWindows
from jupyter_client.session import (
    Session, session_flags, session_aliases,
)
from server.queue.zmqshell import ZMQInteractiveShell
...
kernel_aliases.update({
    'ip' : 'ServerApp.ip',
    'hb' : 'ServerApp.hb_port',
    'shell' : 'ServerApp.shell_port',
    'iopub' : 'ServerApp.iopub_port',
    'stdin' : 'ServerApp.stdin_port',
    'control' : 'ServerApp.control_port',
    'f' : 'ServerApp.connection_file',
    'transport': 'ServerApp.transport',
})

kernel_flags = dict(base_flags)
kernel_flags.update({
    'no-stdout' : (
            {'ServerApp' : {'no_stdout' : True}},
            "redirect stdout to the null device"),
    'no-stderr' : (
            {'ServerApp' : {'no_stderr' : True}},
            "redirect stderr to the null device"),
    'pylab' : (
        {'ServerApp' : {'pylab' : 'auto'}},
...
class ServerApp(BaseIPythonApplication, InteractiveShellApp, ConnectionFileMixin):
  name = 'meta-kernel'
  kernel_name = 'meta-kernel'
  aliases = Dict(kernel_aliases)
  flags = Dict(kernel_flags)
  classes = [MetaKernel, ZMQInteractiveShell, ProfileDir, Session]

  # the kernel class, as an importstring
  kernel_class = Type('server.kernel.metakernel.MetaKernel',
                      klass='server.kernel.base.Kernel',
                      help="""The Kernel subclass to be used.

    This should allow easy re-use of the ServerApp entry point
  ...
  subcommands = {
    'install': (
      'server.kernel.spec.InstallMetaKernelSpecApp',
      'Install the Meta kernel'
    ),
  }
  ...
  def init_extensions(self):
    super(ServerApp, self).init_extensions()
...
launch_new_instance = ServerApp.launch_instance

def main():
  """Run a MetaKernel as an application"""
